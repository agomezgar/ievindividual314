 ...
 */
 ...
 $notas=mysql_query("SELECT * FROM notasindividuales WHERE idalumno='$matriculaAlumno' ORDER BY materia",$link);
...
$texto='';
 ...
 $pdf->SetFont('Arial','',12);
    ...
    $texto=$texto."Materia: ";
    $materiaActual=$notasindividuales["materia"];
    $buscaMateria=mysql_query("SELECT * FROM materias WHERE materia='$materiaActual'",$link);
    while($materiaBuscada=  mysql_fetch_array($buscaMateria)){
        $materiaEncontrada=$materiaBuscada["nombre"];
...
*/
    ...
    $pdf->Multicell(160,10,  $texto,1,'L',0);
    $pdf->Ln(5);
}
